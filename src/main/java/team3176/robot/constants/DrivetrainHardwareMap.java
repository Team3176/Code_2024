...
public class DrivetrainHardwareMap {
  ...
  public static final SwervePodHardwareID POD009 = new SwervePodHardwareID(15, 16, 44.824);//-358.330);
  ...
  // The swerve pod offset is measured when the swerve pod is in the front right position and the
  // wheel gear is facing the right
  // to counteract the offset caused by the mounting in different positions
  public static final double[] AZIMUTH_ABS_ENCODER_OFFSET_POSITION = {
    FR.OFFSET + 180, FL.OFFSET + 90, BL.OFFSET, BR.OFFSET - 90
  }; // TODO: I think these offsets are wrong. I would double check the pods by setting them each as
  // if they were FR zero and remeasuring the offsets
